...
class searchnet:
    ...
    def getallhiddenids(self,wordids,urlids):
      ...
      for urlid in urlids:
        cur=self.con.execute(
        'select fromid from urlhidden where toid=%d' % urlid)
        for row in cur: l1[row[0]]=1
      return list(l1.keys())
...
if __name__=="__main__":
    ...
    # s.trainquery([wWorld,wBank],[uWorldBank,uRiver,uEarth],uWorldBank)
    # result = s.getresult([wWorld,wBank],[uWorldBank,uRiver,uEarth])
    # print(result)

    allurls = [uWorldBank,uRiver,uEarth]
    for i in range(30):
        s.trainquery([wWorld,wBank],allurls,uWorldBank)
        s.trainquery([wRiver,wBank],allurls,uRiver)
        s.trainquery([wWorld],allurls,uEarth)
    result = s.getresult([wWorld,wBank],allurls)
    print(result)